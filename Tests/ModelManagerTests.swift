...
class MockModelManager: ObservableObject {
    ...
    nonisolated func downloadModel(_ model: WhisperModel) async throws {
        ...
        // Simulate download progress with delay
        await MainActor.run {
            self.downloadProgress[model] = 0.5
        }
        ...
        case .large:
            sizeInBytes = Int64(2.9 * 1024 * 1024 * 1024)
        case .largeTurbo:
            sizeInBytes = Int64(1.5 * 1024 * 1024 * 1024)
        }
...
class ModelManagerTests: XCTestCase {
    ...
    func testDownloadModelSuccess() async {
        ...
        let progressExpectation = XCTestExpectation(description: "Download progress should update")
        let progressValues = ActorBox<[Double]>([])
        
        await MainActor.run {
            mockModelManager.$downloadProgress
                .sink { progress in
                    if let modelProgress = progress[model] {
                        Task {
                            await progressValues.append(modelProgress)
                            if modelProgress == 1.0 {
        ...
        do {
            ...
            let values = await progressValues.value
            XCTAssertTrue(values.contains(0.0))
            XCTAssertTrue(values.contains(1.0))
