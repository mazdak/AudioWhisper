...
class SpeechToTextService: ObservableObject {
    ...
    func transcribe(audioURL: URL, provider: TranscriptionProvider, model: WhisperModel? = nil) async throws -> String {
        ...
        case .valid(let audioInfo):
            // Log audio file info for debugging
            print("Validated audio file: \(audioInfo.durationString), \(audioInfo.fileSizeString), \(audioInfo.sampleRate)Hz")
    ...
    private func transcribeWithLocal(audioURL: URL, model: WhisperModel) async throws -> String {
        do {
            return try await localWhisperService.transcribe(audioFileURL: audioURL, model: model) { progress in
