...
struct ContentView: View {
    ...
    private func enhanceProgressMessage(_ message: String) -> String {
        guard let startTime = transcriptionStartTime else { return message }
        ...
        // Add helpful context based on message content
        if message.contains("Preparing") {
            return "\(message) (\(timeString))"
        } else if message.contains("model") {
            return "\(message) - \(timeString) elapsed"
        } else if message.contains("Transcribing") {
            return "\(message) - \(timeString) elapsed"
        } else {
            return "\(message) - \(timeString)"
        }
