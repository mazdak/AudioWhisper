            ...
            for file in audioFiles {
                do {
                    ...
                    if let creationDate = attributes[.creationDate] as? Date, creationDate < cutoffDate {
                        ...
                        print("Cleaned up old recording file: \(file.lastPathComponent)")
                    }
                } catch {
                    print("Failed to clean up file \(file.lastPathComponent): \(error)")
                }
            }
        } catch {
            print("Failed to clean up temporary files: \(error)")
        }
    }
}
